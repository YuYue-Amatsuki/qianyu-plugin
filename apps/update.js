...
export class update extends plugin {
  ...
  async runUpdate(isForce) {
    ...
    this.oldCommitId = await this.getcommitId("qianyu-plugin");
    uping = true;
    ...
    /** 获取插件提交的最新时间 */
    let time = await this.getTime("qianyu-plugin");

    if (/(Already up[ -]to[ -]date|已经是最新的)/.test(ret.stdout)) {
      await this.reply(`千羽插件已经是最新版本\n最后更新时间：${time}`);
    } else {
  ...
  async getLog(plugin = "") {
    ...
    log = await this.makeForwardMsg(`千羽插件更新日志，共${line}条`, log, end);
